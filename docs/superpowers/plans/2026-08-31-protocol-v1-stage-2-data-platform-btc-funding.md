...
**Status:** S01-C `ACCEPTED` and merged (PR #23, merge commit `2ec4917`; executed directly by Hermes). **S01 is complete: A, B and C are all accepted, so BTC settled funding is fully checksum-verified, canonical, immutably published, graph-verified and a verified no-op rerun.** One commit `3392336` adding exactly the one tracked file the Stop C allowlist permits, `docs/superpowers/audits/protocol-v1/btc_settled_funding-publication.md` (236 lines); publication artifacts live under the configured data root and are deliberately not staged into Git, since `/data/` is gitignored. No approval record was required or created: S01-B returned provisional quality `PASS` on all 60 periods with zero `warn` and zero `fail` findings, so there is no approver, no decision time and nothing `authorized: true`. Publication into the real production root `D:/PROJECT/Quantara/data`: **60 PUBLISHED**, 120 HTTP requests (archive plus adjacent `.CHECKSUM` per period), **0 requests outside the frozen allowlist**, exit 0, `stopped_at` null, `workers=1`. Preflight refused to start unless the S01-B inventory was uniformly `PASS` and no `btc_settled_funding` lane already existed (`existing_series_lanes=[]`), so a first publication could not overwrite an existing series pointer. The publish invocation completed all 60 periods and then its own evidence collector aborted on a stale parse-attempt file left by an earlier smoke test — `parse_scalar_rows` refuses to overwrite an existing attempt path — which affected nothing published but lost the run self-report; verification was therefore redone independently from the published production objects by a separate script rather than trusting that report: **60/60** authenticated commit graphs, `current.json` commit matching the verified graph 60/60, provider `.CHECKSUM` digest equal to the retained ZIP sha256 equal to graph `source_sha256` 60/60, member sha256 equal to `parser_input_sha256` 60/60, quality recomputed from published bytes to `PASS` 60/60 with the recomputed `quality_identity` matching the published one 60/60, **0 duplicates, 0 same-key conflicts**, source byte order strictly increasing in every period, object ref kinds `checksum`/`normalized`/`raw` present per period, publication protocol version `v1` on all 60 pointers. Aggregate: **5481 source rows**, 60 distinct commits, 60 distinct content hashes, first settlement 2020-01-01T00:00:00Z, last 2024-12-31T16:00:00Z, 2403 jitter rows preserved; `commit == canonical_content_hash` for all 60 periods, the content-addressed commit scheme behaving as designed. Verified no-op rerun of the whole inventory: **`VERIFIED_NO_OP` x 60, exit 0, ZERO HTTP requests** — no re-download — with commit, `canonical_content_hash` and `manifest_sha256` byte-identical to the publish phase (`IDENTITIES_STABLE True`). Cross-check against S01-B: **0 identity mismatches of 60** on `zip_sha256`, `member_sha256`, `source_rows`, `canonical_content_hash` and `quality_identity`, so publishing to the production root reproduced the temporary-root result exactly and the content address is a pure function of the source bytes. **The S01-A/S01-B production-pointer caveat is now CLOSED by measurement**, satisfying phase-gate item 7 with evidence rather than assertion: the data root was hashed file-by-file immediately before and after publishing — 1408 → 2428 files, 25 → 85 pointers — with **0 pre-existing files changed in size or digest, 0 pre-existing files missing, 0 pre-existing pointers moved**, 60 new pointers (exactly one per period), and **0 files added outside `datasets/series/` and `attempts/`**; the pre-existing pointers include the old BTC kline `1m`/`1h`/`1d` 2020-2024 lanes plus research, training, validation and evaluation, all confirmed unmoved by digest. Of 1020 added files, 900 are the new series lane and 120 are diagnostic attempt manifests. Verification: ruff clean on all S01-C scripts; full suite `-n 4` **2107 passed, 1 skipped** in CI (11m39s); funding-rate values grep-verified absent from the committed document and the evidence JSON; document is LF with no trailing whitespace. Residual risk carried from D09 and restated: parallel acquisition remains unreliable against this provider, so full backfills including this publication run `workers=1`; and it is still not verified whether other sources would hit the same provider behaviour, since only this series has been acquired.) Previously: D09

**Status:** D09 `ACCEPTED` and merged (PR #22, merge commit `b526dd0`; executed directly by Hermes as the correction packet for finding F-S01B-1, which is now `CLOSED`). One commit `967a52b` touching exactly three tracked files: `src/quantara/acquisition.py`, `src/quantara/series_acquisition.py`, `tests/test_acquisition.py`. Both acquisition layers previously decided retry eligibility by substring-matching the exception message for the literal word `reset`, so `httpx.RemoteProtocolError('Server disconnected without sending a response.')` received zero retries. Replaced by one shared public classifier `transport_retry_kind` keyed on exception type, called from both `Acquirer._download` and `SeriesAcquirer._fetch`, eliminating the duplicated policy. `RemoteProtocolError` and `ConnectError` are now retry-eligible; the three eligible timeout classes keep their existing eligibility; the `reset` substring branch is retained only as a widening fallback for `TransportError` subclasses outside the eligible set. `MAX_ATTEMPTS` (3), backoff, and deterministic HTTP status handling are unchanged. A 12-case classifier matrix confirms the widening did not make every transport error retryable: `UnsupportedProtocol`, `ProxyError`, and a non-reset `ReadError` are all `TransportError` subclasses and all remain non-retryable with empty `retry_evidence`. Negative control was behavioural, not deletion-based: rebinding `transport_retry_kind` to the exact pre-D09 semantics while keeping the symbol turns the new tests red on their assertions (`assert 1 == 3`, `assert False`, pytest exit 1) and green when restored, whereas stashing the module would only have produced an `ImportError`. Live re-run against `data.binance.vision`: pre-fix `workers=4` published 1 period and failed 3 with `retry_evidence: []`; post-fix `workers=4` published **19** and then failed 2021-05 after three genuine attempts with recorded `dropped_connection` kinds and 1.070s/2.215s backoff; post-fix `workers=1` published **60/60**, 120 requests, 0 rejected, exit 0. Honest limitation recorded in the packet report: D09 does not make parallel acquisition reliable — Binance drops connections faster than three attempts absorb under concurrency — it makes the failure classified, retried and evidenced instead of silent, and `workers=1` remains the supported mode for full backfills; raising the attempt budget or adding concurrency limits/jitter would be a separate policy change. Published identity is unaffected: the 60-period inventory recomputed after the fix matches the S01-B table on main field-for-field with **0 differences** across rows, `zip_sha256`, `member_sha256`, jitter counts and quality state, and `retry_evidence` appears only in diagnostic attempt manifests written by `new_attempt_manifest`, never as an input to `canonical_content_hash`, `put_object`, `quality_identity`, or any publication manifest, so renaming the series layer's generic `'transport'` label to a specific kind cannot move any published identity. Verification: full suite `-n 4` **2107 passed, 1 skipped** locally (762.61s) and identically in CI (11m15s), ladder 2097 → 2107 for +10 new offline tests; focused `test_acquisition.py` plus `test_series_acquisition.py` 125 passed; fresh `git archive` checkout 125 passed; ruff clean; all 12 frozen S01-A/S01-B pin paths byte-identical to the baseline; no production `data/` writes. The single suite warning is pre-existing (`test_archive.py` duplicate-ZIP-member fixture) and present in the 2097-test baseline.) Previously: S01-B

**Finding F-S01B-1 — `CLOSED` by D09 (PR #22, merge `b526dd0`).** Retained for the record: the defect was a robustness weakness in shared acquisition, never a data-integrity problem, and no incorrect data was produced or published. Residual risk after closure: parallel acquisition is still not reliable against this provider, so full backfills run `workers=1`. Original detail follows.
...
**Gate met for the BTC funding reference slice on 2026-09-05.** D00–D07 accepted (plus D08 and D09 as corrections), S01-A accepted (PR #20 `6708163`), S01-B accepted (PR #21 `a3e6e46`), S01-C accepted (PR #23 `2ec4917`). Evidence: all 60 periods published to the production root with provider checksums matched 60/60, parser-input hashes matched 60/60, quality PASS 60/60 recomputed independently, 5481 rows with 0 duplicates and 0 conflicts, rerun `VERIFIED_NO_OP` x 60 with zero HTTP requests and stable identities, and 0 pre-existing production files or pointers moved. Shared abstractions: full suite 2107 passed, 1 skipped. Remaining Stage 2 scope is the other 12 frozen source series, S02–S13; the stage as a whole is not complete until those are accepted.
