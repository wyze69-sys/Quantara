...
**Status:** IN PROGRESS — C1 `ACCEPTED` and merged (PR #5, merge commit `9a9196e`); C2 `ACCEPTED` and merged (PR #6, merge commit `7abce82`); C3 `ACCEPTED` and merged (PR #7, merge commit `b02cbc5`); C4 is `NEXT`; C5 not started
**Date:** 2026-09-01
...
| C4 | Archive-specific OI timestamp resolution or conservative unknown-role handling, exact final pre-2025 refit sample and failure state, sealed 2026 target-only endpoint buffer, one-year 2025 `REPLICATED` gate | B7, B8, B9, HIGH OI finding | `NEXT` |
