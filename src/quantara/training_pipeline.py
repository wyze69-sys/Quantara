"""Authenticated publication pipeline for exact-decimal ridge training."""
...
from quantara.training_descriptor import TrainingDescriptor, load_training_descriptor
...
def run_training_pipeline(
    ...
    try:
        ...
        report = evaluate_training_quality(
...
def _authenticate_retained_training_commit(
    ...
    if (
        ...
        or len(findings) != len(CHECK_IDS)
        or [item.get("check_id") for item in findings] != list(CHECK_IDS)
    ...
    expected_schema = training_schema_fingerprint(
        _retained_parent_fingerprint(data_root, manifest, "validation")
    )
