...
PROGRESS = EVIDENCE / 'inventory-progress-d10.json'
...
def inspect_period(descriptor, period: str) -> dict:
    ...
    if record['terminal_state'] == 'FAILED':
        ...
        return {
            ...
            'checksum_matches_zip': sha(raw) == acquired['official_digest'] == acquired['raw_sha256'],
...
def main() -> None:
    ...
    for index, period in enumerate(periods, 1):
        ...
        print(f'[{index}/{len(periods)}] {period} {entry["terminal_state"]} '
              f'rows={entry.get("source_rows", "n/a")} distinct={entry.get("distinct_rows", "n/a")} '
              f'dup={entry.get("duplicate_rows", "n/a")} findings='
              f'{[f["check_id"] for f in entry.get("nonpass_findings", [])]}', flush=True)

    inventory = [by_period[p] for p in periods]
    (EVIDENCE / 'inventory-d10.json').write_text(
